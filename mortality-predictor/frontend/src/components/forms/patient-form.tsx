...
import { COMORBIDITY_OPTIONS, OUTCOME_LABELS } from "@/types/patient";
  ...
  return (
    <div>
      <label className="block text-xs text-muted mb-1">
        {label} {unit && <span className="text-zinc-500">({unit})</span>}
      </label>
      <input
        type="number"
        value={value}
  ...
  return (
    <div className="border border-border rounded-lg bg-card">
      <button
        ...
        className="w-full px-4 py-3 flex items-center gap-2 text-left hover:bg-card/80"
      >
...
export function PatientForm({ onSubmit, loading }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const vitals: Vitals = cleanPayload({
      ...
      systolic_bp: n(sbp),
      diastolic_bp: n(dbp),
      mean_arterial_pressure: n(map),
      respiratory_rate: n(rr),
      temperature: n(temp),
      spo2: n(spo2),
      fio2: n(fio2),
      gcs_total: n(gcs),
      urine_output_24h: n(uo),
    }) as Vitals;
    ...
    const labs: Labs = cleanPayload({
      ...
      potassium: n(k),
      sodium: n(na),
      bicarbonate: n(bicarb),
      bilirubin_total: n(bili),
      ast: n(ast),
      alt: n(alt),
      albumin: n(albumin),
      inr: n(inr),
    ...
    const diagnosis: Diagnosis = {
      ...
      is_postoperative: isPostop,
    };

    const interventions: Interventions = {
      on_vasopressors: onPressors,
      vasopressor_count: parseInt(pressorCount) || 0,
      vasopressor_names: [],
      ...
      on_ecmo: onEcmo,
      recent_surgery_within_72h: isPostop,
      central_line: false,
      on_anticoagulation: onAnticoag,
      on_antibiotics: onAbx,
    };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              required
              placeholder="e.g. Sepsis, Acute MI, Pneumonia, AKI..."
              className="w-full bg-background border border-border rounded-md px-3 py-2 text-sm
                         focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
            />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <label className="flex items-center gap-2 text-sm pt-5">
              <input
                type="checkbox"
                checked={isSurgical}
                onChange={(e) => setIsSurgical(e.target.checked)}
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs border transition-colors ${
                    comorbidities.includes(opt.value)
                      ...
                      : "bg-background border-border text-muted hover:border-zinc-500"
                  }`}
                >
                  {opt.label}
                </button>
      ...
      <Section title="Vital Signs" icon={Heart} defaultOpen={false}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <NumberInput label="SpO₂" unit="%" value={spo2} onChange={setSpo2} />
          <NumberInput label="FiO₂" unit="0.21-1.0" value={fio2} onChange={setFio2} step={0.01} />
      ...
      <Section title="Laboratory Values" icon={TestTube} defaultOpen={false}>
        <div className="space-y-3">
          <p className="text-xs text-muted">Renal</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <p className="text-xs text-muted pt-2">Hepatic</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <p className="text-xs text-muted pt-2">Hematologic</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <NumberInput label="WBC" unit="×10³/µL" value={wbc} onChange={setWbc} step={0.1} />
            <NumberInput label="Platelets" unit="×10³/µL" value={plt} onChange={setPlt} />
          </div>
          <p className="text-xs text-muted pt-2">Metabolic / Inflammatory</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <p className="text-xs text-muted pt-2">Cardiac</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <p className="text-xs text-muted pt-2">ABG</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <NumberInput label="pH" value={ph} onChange={setPh} step={0.01} />
            <NumberInput label="PaO₂" unit="mmHg" value={pao2} onChange={setPao2} />
            <NumberInput label="PaCO₂" unit="mmHg" value={paco2} onChange={setPaco2} />
          </div>
        </div>
      </Section>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={onPressors}
                onChange={(e) => setOnPressors(e.target.checked)}
                className="rounded border-border"
              />
              On Vasopressors
            </label>
          ...
          <div>
            ...
            <select
              ...
              className="w-full sm:w-1/3 bg-background border border-border rounded-md px-3 py-2 text-sm
                         focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
            >
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={onEcmo}
                onChange={(e) => setOnEcmo(e.target.checked)}
                className="rounded border-border"
              />
              On ECMO
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={onAbx}
                onChange={(e) => setOnAbx(e.target.checked)}
                className="rounded border-border"
              />
              On Antibiotics
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={onAnticoag}
                onChange={(e) => setOnAnticoag(e.target.checked)}
                className="rounded border-border"
              />
              On Anticoagulation
            </label>
